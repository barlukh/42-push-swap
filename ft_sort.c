...
/*   Updated: 2025/05/30 10:26:23 by bgazur           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_sort(t_stacks *stack)
{
	...
	ft_median(stack);
	ft_min_max(stack);
	while (stack->j < (size_t)stack->median)
	{
	...
	while (stack->b[0] != stack->median && stack->a[0] != (int)stack->median + 1)
	{
		if ((stack->b[0] != stack->median) && (int)stack->median + 1)
			ft_ops_main(RR, stack);
		else if (stack->b[0] != stack->median) // Largest value of 'b' (max of smaller half)
			ft_ops_main(RB, stack);
		else if (stack->a[0] != (int)stack->median + 1) // Smalles value of 'a' (min of bigger half)
